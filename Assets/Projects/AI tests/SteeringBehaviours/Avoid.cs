...
public class Avoid : SteeringBehaviour_Base
{
	...
	private void FixedUpdate()
	{
		...
		if (didItHitAnything)
		{
			...
			if (rb != null)
			{
				float hitInfoDistance = ((maxDistance - hitInfo.distance) / maxDistance);
				// Debug.Log(gameObject.name + " : "+ hitInfoDistance);
				rb.AddTorque(0,turnSpeed * turnForce * hitInfoDistance,0, ForceMode.Impulse);
				rb.AddRelativeForce(0,0, (-hitInfoDistance - 1f )* 100f);
			}
		}
		else
		{
